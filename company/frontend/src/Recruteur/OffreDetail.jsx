...
//   console.log('OffreDetail props:', offre.offre.id);
...
//     setQuestions(prev => [
//       ...prev,
//       { texte: newQuestionText, niveau: levelStep, duree_limite: 60 }
//     ]);
//     setNewQuestionText('');
//   };
...
//     try {
//       console.log('Envoi des questions au backend:', questions);
//       const response = await fetch(`http://localhost:8000/api/recruteur/offres/${offre.offre.id}/questions`, {
//         method: 'POST',
...
//     setOpenMenu(null);
//     try {
//       const response = await fetch(`http://localhost:8000/api/candidat/${candidatId}/update-status/`, {
//         method: 'PATCH',
//         headers: { 'Content-Type': 'application/json' },
...
//       setCandidats(prev =>
//         prev.map(c => c.id === candidatId ? { ...c, status: updatedCandidat.status } : c)
//       );
//       if (onStatusChange) onStatusChange(candidatId, updatedCandidat.status);
...
//       method: 'POST',
//       headers: { 'Content-Type': 'application/json' },
//       body: JSON.stringify({ offre_id: offre.offre.id, deadline : deadline })
//     })
//       .then(res => res.json())
//       .then(data => {
//         if (data.error) {
...
//           // Afficher chaque lien dans une alerte
...
//     return ['Retire', 'Pending', 'Preselectionne', 'Entretient', 'En Processus d\'entretien', 'En negotiation', 'Recrute'];
//   };
...
//       {/* --- Left column --- */}
//       <ColumnLeft>
//         <h1>{offre?.offre?.titre || 'Titre indisponible'}</h1>
//         <p>{offre?.offre?.description || 'Description indisponible'}</p>
//         <p>📍 Lieu: {offre?.offre?.lieu || 'Non spécifié'}</p>
//         <p>📄 Type de contrat: {offre?.offre?.typecontrat || 'Non spécifié'}</p>
//         <p>💼 Métier: {offre?.offre?.metier || 'Non spécifié'}</p>
//         <p>Salaire : {offre?.offre?.salaire || 'Non spécifié'}</p>
//       </ColumnLeft>
...
//               {questions.filter(q => q.niveau === levelStep).length < 5 && (
//                 <>
...
//                         <span><strong>{q.niveau}:</strong> {q.texte} ({q.duree_limite}s)</span>
...
//   const [candidats, setCandidats] = useState([]); // Initialize as empty array
...
//   // Ensure filteredCandidats is always an array
...
//         texte: newQuestionText, 
//         niveau: levelStep, 
//         duree_limite: questionTime * 60 // Convert minutes to seconds
//       }
//     ]);
//     setNewQuestionText('');
//   };
...
//                           <div>
//                             <QuestionLevel>Niveau: {q.niveau}</QuestionLevel>
//                             <QuestionTime>Temps: {q.duree_limite / 60} minutes</QuestionTime>
//                           </div>
//                         </QuestionInfo>
//                         <RemoveButton onClick={() => removeQuestion(indexToRemove)}>✖</RemoveButton>
//                       </QuestionItem>
...
//       })
//       .then(data => {
//         setOffre(data); // attention: ta vue renvoie { offre: {...} }
//         setLoading(false);
//         console.log("Offre chargée:", data);
//       })
//       .catch(err => {
...
//       })
//       .then(data => {
//         setCandidats(data); // stocke séparément
//         setLoading(false);
...
//   if (loading) return <p style={{ textAlign: 'center', padding: '2rem' }}>Chargement...</p>;
//   if (!offre) return <p style={{ textAlign: 'center', padding: '2rem' }}>Offre introuvable</p>;
...
//           </thead>
//           <tbody>
//             {filteredCandidats.map(candidats => (
//               <tr key={candidats.id}>
//                 <td>{candidats.nomComplet}</td>
//                 <td>{candidats.universite}</td>
//                 <td>{candidats.diplome}</td>
//                 <td>{candidats.filiere}</td>
//                 <td>{candidats.matchingScore}</td>
//                 <td>{candidats.dateCandidatsure}</td>
//                 <td><StatusBadge status={candidats.status}>{candidats.status}</StatusBadge></td>
//                 <td>
//                   <ActionMenu>
//                     <MenuButton onClick={() => toggleMenu(candidats.id)}><FaEllipsisH /></MenuButton>
//                     <MenuContent show={openMenu === candidats.id}>
//                       {getAvailableTransitions(candidats.status).map(action => (
//                         <MenuItem key={action.status} onClick={() => handleStatusChange(candidats.id, action.status)} variant={action.variant}>
//                           {action.icon} {action.label}
//                         </MenuItem>
//                       ))}
//                     </MenuContent>
