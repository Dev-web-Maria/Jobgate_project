...
const Instructions = () => {
  ...
  const nomEntreprise = "Entreprise"; 
  ...
  // ===== Démarrage "one-shot" : consommer le lien AVANT de naviguer =====
  const handleStartInterview = async () => {
    ...
    try {
      const resp = await fetch(`${API_BASE}/${token}/start/`, {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      // Gestion des cas serveurs normalisés :
      // 200 = OK (PENDING -> IN_PROGRESS), 409 = déjà utilisé/démarré/terminé, 410 = expiré
      if (resp.status === 200) {
        // on a consommé le lien, on enchaîne vers la vraie page d’entretien
        navigate(`/candidat/dashboard/messagerie/instructions/interview/${token}`);
        return;
      }

      // Autres statuts -> afficher un message clair
      let payload = null;
      try { payload = await resp.json(); } catch {}
      const serverMsg = payload?.message || payload?.error || "";

      if (resp.status === 409) {
        setError(serverMsg || "Ce lien d'entretien a déjà été utilisé ou l'entretien est déjà démarré.");
      } else if (resp.status === 410) {
        setError(serverMsg || "Ce lien d'entretien est expiré.");
      } else if (resp.status === 400) {
        setError(serverMsg || "Impossible de démarrer l'entretien (requête invalide).");
      } else {
        setError(serverMsg || "Une erreur est survenue lors du démarrage de l'entretien.");
      }
    } catch (e) {
      console.error("Erreur lors du démarrage de l'entretien:", e);
      setError("Problème de communication avec le serveur. Veuillez réessayer.");
    } finally {
      setIsLoading(false);
    }
  };

  // Entretien d'essai : NE CONSOMME PAS le lien (pas d'appel /start/)
  const handleStartTestInterview = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/${token}/statut/`, {
        method: 'GET',
        credentials: 'include',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token') || ""}`,
      ...
      const statut = j?.data?.statut?.toUpperCase?.() || "";

      if (statut === 'EXPIRED' || statut === 'EXPIRE') {
        setError(`Cet entretien a expiré. Vous ne pouvez pas faire l'entretien d'essai.`);
        return;
      }
      if (statut === 'COMPLETED' || statut === 'TERMINE') {
        setError('Vous avez déjà terminé cet entretien.');
        return;
      }

      // sinon lancer l’entretien d’essai
      navigate(`/candidat/dashboard/messagerie/TestInterview`, { state: { token } });
    } catch (error) {
      console.error('Erreur lors du lancement de l’entretien test:', error);
      setError('Impossible de démarrer l’entretien d’essai. Veuillez réessayer.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{
          ...
          <div style={{
            ...
            >
              <button
                onClick={handleStartTestInterview}
